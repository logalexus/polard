  ...
  <v-container fluid class="d-flex flex-row pa-2" style="min-height: 100vh; gap: 8px;">
    <v-card class="flex-1-1-100 pa-4" style="height: 100vh;">
      <v-card-title class="d-flex justify-center text-h5 font-weight-bold">
        <span>Тестирование публичного ключа RSA</span>
      </v-card-title>
      ...
      <template v-else>
        <v-btn color="primary" class="mt-4" @click="checkKey">Старт</v-btn>
      </template>
    </v-card>
    <v-card class="flex-1-1-100 pa-4" style="height: 100vh;">
      <v-card-title class="d-flex justify-center text-h5 font-weight-bold">
...
export default {
  data() {
    return {
      ...
      loading: false,
      headers: [
        { title: 'Статус', align: 'start', value: 'status', headerProps: {style: 'font-weight: 700'} },
        { title: 'Тест', align: 'end', value: 'description', headerProps: {style: 'font-weight: 700'}  },
      ],
      tests: [],
    };
  },
